...
- 展示"即将开始的比赛"列表
- 展示"待录入成绩提醒"：当有已结束但未录入成绩的比赛项目时，顶部显示黄色提醒卡片，点击项目名称可直接跳转到成绩录入页面
- 展示"各院系报名分布"环形图：直观展示各院系的报名人次占比
- 可作为系统首页检查当前数据概况
...
| 结束比赛 | 点击"结束"按钮，确认后项目状态变为"已结束"，自动跳转到该项目的成绩录入页面，提示录入获奖运动员成绩 |
...
| 新增报名 | 点击"新增报名"，选择运动员（显示性别）和比赛项目（仅显示可报名项目，已结束项目不显示）|
| 无可报名项目 | 当所有项目均已结束时，项目下拉显示红色提示"当前没有可报名的项目" |
...
| 结束后录入 | 比赛项目结束比赛后自动跳转到该项目的成绩录入页面，页面顶部显示提醒横幅 |
...
| 运动员 | 20 名（10 男 10 女，分布在 6 个院系）|
| 报名记录 | 26 条（覆盖 6 个项目，部分运动员跨项目报名）|
| 成绩记录 | 10 条（男子100米 5 名 + 女子100米 5 名）|
